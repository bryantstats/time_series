...
- The correlation between a time series $y_t$ and the yesterday's version of itself $y_{t-1}$ is call autocorrelation at lag 1. 
...
- The Autocorrelation function (ACF) is the function where the input is a lag and the output is the autocorrelation of the lag. 
...
- A Stationary series where both the mean and variance stay constant. 
...
- This is a plot of a Random Walk series. The series is not stationary because the expected value (the mean) is changing over time. 
...
- For example: below is the ACF of a linear trend (hence, not stationary) series
...
- Below is the ACF of a time series with seasonality (hence, not stationary). 
